//Cecil - v. 1.0, art rhyno, conifer/hackforge/ourdigitalworld
//    Project Conifer serializer for tiles, using JAI
//    July 2009
//
//  Rev: Jan 2013 - Cocoon Trunk
//       Mar 2013 - bumped Lucene support up to 4.2.0 
//       Oct 2014 - added http option for openstack support   

package org.conifer.odw.components.serializers;
    ...
    implements Configurable, Disposable, Serviceable 
{
    public final String TILE_NAMESPACE = "http://ourontario.ca/tile/1.0";
        ...
        //this is arbitrary but would get silly beyond this point
        int MAX_HITS_PER_PAGE = 500;
    ...
    */
    ...
    protected void createTile(Attributes atts) throws SAXException {
                //System.out.print("JVM verion: " + System.getProperty("sun.arch.data.model"));
        ...
        try {
            ...
            //JAI woes can slip through the cracks, check for a problem with readers
            try {
                reader = (ImageReader)readers.next();
            } catch (Exception ex) {
                System.out.println("JAI READER EXCEPTION: " + ex.toString());
            }//try

            ImageInputStream iis = null;
                        
            //both openstack and S3 would use URI-addressible sources
            if (src.startsWith("http://")) {
                InputStream is = new URL(src).openStream();
                iis = ImageIO.createImageInputStream(is);
            } else {
                iis = ImageIO.createImageInputStream(new File(src));
            }//if

            //Another potential pain point for JAI
            try {
                reader.setInput(iis,false,true);
            } catch (Exception ex) {
                System.out.println("IMAGEIO EXCEPTION: " + ex.toString());
            }//try
            ...
            try {
                ...
                System.out.println("READER WITH PARM EXCEPTION: " + ex.toString());
            }//try
    ...
    public Vector<Document> getResults(String search, String indexsrc)
    {
        ...
        try {
            ...
            Query query = new QueryParser(Version.LUCENE_40, "word", analyzer).parse(search);
    ...
    public String getHlString(int width, int height, String query, String indexsrc, int[] imgcoords)
    {
        ...
        Vector<Document> wordMatches = getResults(query,indexsrc);
                System.out.println("wordmatches res: " + wordMatches.size());
        ...
        */
        ...
        //our next step is to try to center them in the area set for display
        x1 = y1 = x2 = y2 = 0;
       ...
       //System.out.println("RESULT SIZE: " + tileDocs.size());
        for (int i=0;i<tileDocs.size();i++) {
            tileDoc = (Document) tileDocs.get(i);
            ...
            //System.out.println(leftPtX + "," + leftPtY + " - " + rightPtX + "," + rightPtY);
    ...
    public BufferedImage revampRaster(Raster tile, ColorModel cm) {
        ...
        WritableRaster wr = tile.
            ...
            dataBuffer, null);
        revImg = new BufferedImage(cm,wr,
