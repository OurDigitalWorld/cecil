/* 
slight modifications for ourontario newspaper application, based on
image reader from cocoon 2.1
...
/**
 * The <code>ImgOpReader</code> component is used to serve binary image data
 ...
 *
 * @version $Id: ImgOpReader.java 607739 2007-12-31 15:22:35Z joerg $
 */
final public class ImgOpReader extends ResourceReader
    ...
    throws ConfigurationException {
        ...
        try {
            ...
            throw new ConfigurationException( "Unable to configure ImgOperations", e );
        }
    }
    ...
    throws ServiceException {
        this.manager = man;
        operationSelector = (ServiceSelector) man.lookup( GenericImgOperation.ROLE + "Selector" );
    }
    ...
    */
    public Serializable getKey() {
        ...
        while( list.hasNext() ) {
            GenericImgOperation op = (GenericImgOperation) list.next();
            b.append( op.getKey() );
            b.append( ':' );
        }
    ...
    throws ConfigurationException, ServiceException {
        ...
        for( int i=0 ; i < ops.length ; i++ ) {
            ...
            GenericImgOperation op = (GenericImgOperation) operationSelector.select( type );
            op.setPrefix( prefix );
            effectsStack.add( op );
        }
    }
    ...
    private BufferedImage applyEffectsStack( BufferedImage image ) throws ProcessingException {
        ...
        while( list.hasNext() ) {
            GenericImgOperation op = (GenericImgOperation) list.next();
            WritableRaster r;
            if (op instanceof ImgOperation) {
                newImage = ((ImgOperation) op).apply( image );
            } else if (op instanceof CombineImgsOperation) {
                CombineImgsOperation cio = (CombineImgsOperation) op;
